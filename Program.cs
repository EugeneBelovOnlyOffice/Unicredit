...
for (int i = 0; i < qadBankArray.Length; i++)
{
    ...
    if (Methods.Class1.ParseString(qadBankArray[i], 110)[82] != "Y")
    {
        ...
        singleOrderArray[11] = _ = Methods.Class1.ClearNonUTF8(Methods.Class1.ParseString(qadBankArray[i], 110)[70] + Methods.Class1.ParseString(qadBankArray[i], 110)[71] + Methods.Class1.ParseString(qadBankArray[i], 110)[72] + Methods.Class1.ParseString(qadBankArray[i], 110)[73]);//Назначение платежа	Ground	{VO20050} Оплата по счету 16 от 11.01.2021 г. за консультационные услуги. НДС не облагается	Да	A210
